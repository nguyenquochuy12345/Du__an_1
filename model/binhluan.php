...
function show_binhluan($id, $so_sanpham_tren1trang, $trang)
{
    ...
    $sql = "SELECT binhluan.binhluan_id,binhluan.user_id,taikhoan.vaitro_id,taikhoan.hovaten , binhluan.ngaybl ,binhluan.noidung  FROM binhluan  JOIN products ON products.product_id = binhluan.product_id JOIN taikhoan on binhluan.user_id = taikhoan.user_id WHERE products.product_id='$id' order by ngaybl desc LIMIT  " . $so_sanpham_tren1trang . " OFFSET " . $offset . " ";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
{
    include './ketnoi/ketnoi.php';
    $sql = "SELECT rep_id, rep.binhluan_id,rep.user_id,taikhoan.hovaten,taikhoan.vaitro_id , rep.ngay_traloi , 
